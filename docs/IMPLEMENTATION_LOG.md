...
|--------|-------------|--------|---------|
| feature/dev-start-sidebar-delete-fix | Dev startup hang + sidebar delete fix — removed the workspace sidebar’s mount-time auto-delete of non-workspace conversations and bounded `dev.sh` HTTP health probes so start/status cannot block indefinitely on localhost curl checks | done | 2026-05-13 |
