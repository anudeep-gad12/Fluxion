...
| main | UI polish pass 2 foundations through tooltips — flattened the terminal panel, unified interaction/focus/panel tokens, removed dead primitives, migrated neutral surfaces across primitives/transcript/shell, consolidated entrance motion, and replaced eligible native control titles with the shared warm-up tooltip primitive | done | 2026-07-06 |
| main | Concurrent-run model isolation tests — `tests/routes/test_agent_runs.py` gains `TestConcurrentRunModelIsolation`: two runs posted with different `X-Provider`/`X-Model` headers each persist their own `model_config_snapshot.selected_provider/selected_model`, and a run with explicit headers resolves its own provider via the registry even when the process-global local override and active model are set (engine kwargs captured through a mocked `create_agent_engine`); full suite 1123 passed | done | 2026-07-06 |
| main | Sidebar live run status — `threadStatusForConversation` gains a `needs-attention` state (amber pulsing dot, "Waiting for your approval" tooltip) derived from the shared `conversationAttention` store helper (pending tool approval / plan approval / user input in any of the conversation's live runs), and running dots now trust live `agentRunState` over stale `run.status`, so backgrounded concurrent runs show accurate status and surface approval prompts without switching into them blind | done | 2026-07-06 |
| main | Per-conversation run gating — the main window no longer locks app-wide while a run is active: ConversationView's `hasActiveRun` now comes from `useConversationHasActiveRun(selectedConversationId)` (composer/retry/rewind/paste/Escape-rewind gate only on the selected conversation's own runs, steer behavior unchanged), New workspace/new conversation actions in App.tsx/ConversationList.tsx and the k→n shortcut lost their global gates entirely, and chat mode gets an explicit single-slot guard (toast when another conversation's chat response is still streaming); `useHasActiveRun` survives only for the FloatingOverlay window | done | 2026-07-06 |
| main | Agent stream manager — replaced the single-slot `useAgentSSE` hook with module-level `ui/src/lib/agentStreamManager.ts` owning one EventSource per active run (idempotent `ensureAgentStream` survives loadConversation re-runs/StrictMode; per-run epoch/lastSeq/stream-token/flush buffers/reconnect; terminal events close + free their slot; mid-run reconnects no longer wipe accumulated steps via unconditional `initAgentRun`); ConversationView and FloatingOverlay rewired (`subscribedRunRef` deleted), so switching conversations no longer tears down a running conversation's stream — N conversations can stream agent runs live concurrently (WKWebView ~6-connections/host caps practical concurrency at ~4-5) | done | 2026-07-06 |
| main | Per-conversation run selectors — `useStore` now maintains a `conversationIdByRunId` reverse map (populated in `setRuns`/`addRun`, cleaned in `removeRun`/`removeConversation`) and exposes `conversationHasActiveRun`/`conversationAttention` plain functions plus `useConversationHasActiveRun`/`useConversationAttention`/`useIsChatStreaming` hooks; live `agentRunState.isActive` wins over possibly-stale `run.status` so backgrounded runs can't wedge gating; groundwork for replacing the global `useHasActiveRun` gate with per-conversation gating | done | 2026-07-06 |
| main | Sticky model selection — new shared `ui/src/lib/modelSelection.ts` (metadata key + `getConversationModelSelection` dedup'd out of ConversationView/FloatingOverlay, sticky last-used selection in localStorage excluding `local` provider, registry-based seed for pre-stickiness installs); draft selection now seeds from sticky so every agent/chat run sends explicit `X-Provider`/`X-Model` headers and legacy conversations get their model pinned into metadata on first send — the backend's process-global `_active_model` fallback is no longer consulted, which is a prerequisite for concurrent conversations not stealing each other's model | done | 2026-07-06 |
| main | Premium-polish phase 5 (motion QA + sweep) — run-message entrance animation now fires only while the run is live (virtualized remounts of settled runs no longer re-animate on scroll); footer placeholder purge completed: `cost n/a` dropped from `getRunFooterMetrics` and `AgentRunMessage`, `compact 0` hidden unless > 0; full-surface screenshot sweep in both themes (home, agent transcript with tool calls, model picker, light/dark via emulated color-scheme) came back clean | done | 2026-07-03 |
| main | Premium-polish phase 4 (keyboard & focus) — new `hooks/useDismissable.ts` (outside pointer-down + Escape) adopted by the run-settings menu and terminal add-menu (context menu already handled Esc; mention picker closes via composer keydown); `ui/dialog.tsx` gains Escape-to-close, a Tab focus loop, focus-into-dialog on open and focus-restore on close (`role=dialog aria-modal`), inherited by all dialogs; `:focus-visible` rings (`var(--focus-ring)`) on every bespoke control that only had `:hover` (`desktop-icon-btn/chip/model-trigger/mode-switch/send-btn/browser-nav/terminal-tab*`, settings buttons/list items, `tr-prompt-btn/thinking-toggle/more`); `.animate-in`/`.animate-pulse` added to reduced-motion; verified live: Esc closes run-settings popover, Tab shows accent ring | done | 2026-07-03 |
| main | Premium-polish phase 3 (component consolidation) — `ui/button.tsx` variants flattened (no more hover-lift/glow that `shell.css` was force-neutralizing with `!important`; neutralizer block deleted; radii onto the scale); **fixed the invisible agent streaming caret** (`desktop-agent-caret` was referenced but never defined — `.agent-caret` now carries size/bg and all 4 caret sites use it); chat-mode waiting state unified onto the braille status-line language (`ShimmerSkeleton`/`ThinkingTimer` + shimmer keyframes deleted, `StreamingIndicator.tsx` removed); icon consistency: unicode `×`→lucide `X`, `✓ copied`→lucide `Check`+label; rewind dialog restyled from one-off zinc mono card onto the standard `desktop-settings-*` dialog vocabulary | done | 2026-07-03 |
| main | Premium-polish phase 2 (chrome & layout pixel pass) — titlebar title centered over the thread column with a 14px gradient scrim below (scrolling text dissolves instead of hard-clipping); composer meta cleaned up: `AgentContextFooter` renders only real metrics (no more `— ctx · — tok · cost n/a` placeholders), char counter hidden until typing (row min-height reserved so nothing jumps); markdown tables shrink-wrap (`w-max max-w-full` kills the empty right band inside the border); code-block radius conflict resolved (AnswerMarkdown wrappers → `rounded-lg` matching the desktop `.markdown pre` 8px); scrollbars 4→6px on `--scrollbar-thumb*` tokens (killed 3 literal color sets); accent-tinted `::selection`; thread scroll gets pb-10; sidebar workspace-header spacing; **vite `base` './'→'/'** — direct deep links (`/conversations/<id>`) previously 404'd their assets and blank-screened | done | 2026-07-03 |
| main | Premium-polish phase 1 (design-token foundations) — tokens.css gains radius (`--radius-xs/sm/md/lg/xl`), elevation (`--shadow-menu/dialog/overlay`, `--glow-accent`, `--focus-ring`) and layering (`--z-raised…--z-dragframe`, documenting the 21474xxxxx Tauri band) scales; all CSS motion literals (`0.12s/0.14s/0.15s/0.16s ease`, `120ms`, foreign `cubic-bezier(0.4,0,0.2,1)`) now use `var(--duration-*) var(--ease-ui)`; ~30 ad-hoc radii and the 3 near-identical modal shadows collapsed onto the scales; TSX dedup: `text-[12px]`→`text-xs`, `text-[14px]`→`text-sm`, `tracking-[0.18em]`→`[0.16em]`, magic z-indexes→`z-[var(--z-*)]`, arbitrary rgba shadows→`var(--shadow-menu)`/`var(--glow-accent)`; syntax-highlighter + meta theme-color read `--desktop-bg-*` instead of hex copies; Inter font straggler and literal mono stacks→`var(--font-sans/mono)`; visual no-op verified on :9000 | done | 2026-07-03 |
| main | Premium-polish phase 0 (web-skin deletion) — the app is now single-path desktop: removed every `isLocalDesktopApp()`/`desktop ? … : …` UI fork (App.tsx owner/demo/mobile system, ModelPicker 73 ternaries, ReasoningSettingsDialog 26 + 5 `<select>` fallbacks, dialog.tsx `bg-[#18181b]`, EmptyState web ⌘-hint, TerminalPanel/ConversationView guards); `WorkspacePickerDialog` reduced to a headless native-picker bridge (350→43 lines); purged dead CSS (`premium-chip/field/panel/primary-button`, `--fluxion-*` aliases, `.desktop-terminal-rail-*`, `.desktop-status-bar`, `.desktop-panel-rail`, `.desktop-settings-provider-count`) and deleted unused `DesktopChipMenu.tsx`; visual no-op verified on :9000 | done | 2026-07-03 |
| main | Braille spinner (final) — comet ring swapped for the terminal braille cycler per user preference: `transcript/BrailleSpinner.tsx` (renamed from CometSpinner) cycles `⠋⠙⠹⠸⠼⠴⠦⠧⠇⠏` at 80ms with accent color + `text-shadow` glow (`.tr-spinner`), freezes to a dim solid `⠿` when idle, and skips the interval entirely under `prefers-reduced-motion` (checked in JS via `matchMedia`); verified both themes/states via built-CSS harness screenshots | done | 2026-07-03 |
| main | Comet arc ring spinner — replaced the Claude-Code starburst (`✳✶✻✽` glyph cycler in `RunStatusLine` + static `✳` waiting line) with a pure-CSS conic-gradient comet ring (`transcript/CometSpinner.tsx`, `.tr-comet` in `transcript.css`): 12px spinning arc with fading tail and accent glow matching the sidebar running dot, frozen neutral ring when idle, covered by `prefers-reduced-motion`; `MarkerLine.marker` widened to `ReactNode`; kills the 300ms JS re-render interval; verified both themes/states via built-CSS harness screenshots (packaged app on :9000 still serves the old bundle until rebuilt) | done | 2026-07-03 |
| main | UI overhaul phase 4 (light pixel pass + dead CSS purge) — light theme gains scrollbar/caret/shimmer overrides so both themes are fully token-driven (verified in-browser: transcript markers, diff tints, sidebar, composer, overlay); deleted dead CSS: `tool-diff.css` (superseded by `tr-diff`), `.fluxion-card*`, `.fluxion-pill/topbar/trace`, `.agent-loader-stroke`, `.animate-indeterminate`, `.collapsible-content`, `.ui-text-*`/`.ui-accent-*` utilities; `desktop-thread.css` split into `transcript.css` (run layout/footer/markdown tints) and new `overlay.css` (floating window) | done | 2026-07-03 |
| main | UI overhaul phase 3c (single flat premium primitives) — the `.premium-chip/field/panel/subtle-button/primary-button/section-label` classes are now defined once in `index.css` as flat token-based primitives (8/12px radius, hairline borders, no shadows/transforms, accent focus ring) and the `[data-app='desktop']` re-override block in `shell.css` is gone; consumers (WorkspacePickerDialog, ModelPicker, ReasoningSettingsDialog, AnswerWithCitations) inherit the native look in both themes | done | 2026-07-03 |
| main | UI overhaul phase 3b (sidebar/titlebar/composer polish) — conversation rows are dense single-line 28px items (status dot + title + hover delete; relative time moved to tooltip), workspace section labels 11px/500, header buttons shrunk to 28px, list rhythm tightened to a 4px grid; titlebar title medium-weight; composer shell radius 16→12px with a soft accent focus ring and an amber border tint while steering an active run | done | 2026-07-03 |
| main | UI overhaul phase 3a (layout unification + composer dedup) — the desktop layout is now the only layout: deleted the web-only branch in `ConversationView` (its `ConversationToolbar` header, `desktop/Composer.tsx` card composer, floating `AgentLiveHUD` mount, queued-steer chips) so `DesktopChrome` + `DesktopInputDock`→`DesktopComposer` render unconditionally; `ChatMode` type moved to `@/types`; dead `AgentComposerOptions` component stripped (only `AgentContextFooter` survives); `.fluxion-composer`, `.desktop-composer-card`, `.desktop-send-button*`, `.desktop-mode-tab*`, `.desktop-ghost-control` CSS purged | done | 2026-07-03 |
| main | UI overhaul phase 2c (status line + inline prompts) — `AgentLiveHUD` is now a thin dispatcher over `transcript/RunStatusLine.tsx` (one `✳ Working… (12s · 3.2k tok · 19% ctx)` spinner line replacing the live card + metric pills) and `transcript/InlinePrompts.tsx` (flat approval/plan/user-input prompts with a 2px kind-colored left rule and plain-text approve/deny buttons; decision handlers moved verbatim incl. the 404/409 optimistic store unblock); `desktop-hud.css` deleted; live approve/deny smoke NOT yet run — Fireworks account suspended (412) blocked agent runs | done | 2026-07-03 |
| main | UI overhaul phase 2b (user turns + thinking flattening) — user messages render as flat `>` lines instead of bordered cards (`transcript/UserTurn.tsx`), chat-mode `ThinkingPanel` dropped its card/badges/live-box for the same dim `⏺ Thinking…` click-to-expand line the agent transcript uses, cancelled/interrupted/failed run states became toned marker lines, and the chat answer stream lost its `fluxion-card-strong` wrapper; matching `.desktop-message-card`/`.desktop-thinking-panel` CSS deleted | done | 2026-07-03 |
| main | UI overhaul phase 2a (CC-style agent transcript) — replaced `AgentStepsPanel` (cyan progress spine, animated status dots, thinking badges) and `ToolCallCard` (bordered cards, `⋯→✓✗` markers) with a flat Claude-Code-style transcript: `transcript/AgentTranscript.tsx` (step grouping preserved verbatim), `transcript/ToolCallLine.tsx` (`⏺ read(path) (2.1s)` lines with status-toned markers), `transcript/TranscriptLine.tsx` primitives (`⎿` result rows clamped to 3 lines with `… +N lines` expanders), `transcript/toolFormat.tsx` (moved helpers + new flat single-column unified diff with 12-row clamp), and `styles/transcript.css`; thinking is now one dim `⏺ Thinking…` line expanding on click; verified against real runs on :9000 | done | 2026-07-03 |
| main | UI overhaul phase 1 (token consolidation) — new `ui/src/styles/tokens.css` is the single source of truth for both themes (desktop ladder + text ramp + accent/status colors on `:root`, one `[data-theme='light']` block replacing the two duplicate light definitions, shadcn HSL vars re-derived from the ladder, SF Pro/SF Mono font tokens, new `--term-*` xterm tokens); killed the web "Fluxion" skin (Google Fonts import, gradient body/app background, blue-tinted HSL palette), renamed `desktop-tokens.css`→`shell.css` (shell chrome only), xterm now builds its theme from `--term-*` via `buildTerminalTheme()`, Tailwind gains `surface/ink/line/accentc` semantic colors and token font stacks; global radius unified to 0.5rem | done | 2026-07-03 |
| main | UI overhaul phase 0 (structural extraction) — split `ConversationView.tsx` (4065→2413 lines) into `ModelPicker.tsx`, `ReasoningSettingsDialog.tsx`, `MentionPicker.tsx`, and `ChatRunMessage.tsx` (renamed RunMessage), moved shared run formatting helpers to `lib/runFormat.ts`, and extracted the user-turn/footer fragments duplicated by chat+agent run renderers into `transcript/UserTurn.tsx` and `transcript/RunFooter.tsx`; zero visual or logic change, groundwork for the hybrid CC-transcript/native-shell redesign | done | 2026-07-02 |
| main | Coding context cache-stability + LLM compaction — moved the mutating session-state metadata block to the end of the prompt so the `[system][checkpoint][restored][transcript]` prefix stays byte-stable for provider prompt caching, bounded tool outputs once at persist time (per-tool budgets ×4 with explicit re-run markers) so stored replay entries never change, gated the in-place reduction stages 1–3 behind `context.coding_legacy_reduction` (default off; pressure now escalates straight to checkpoint compaction), and compaction checkpoints are now written by the active model via a Codex-style handoff prompt (`compaction_summarizer.py`) with the heuristic summary as silent fallback and `summary_source`/duration in the compaction trace | done | 2026-07-02 |
| main | Floating overlay sizing + model picker UX — window bounds now match the visible card (removed the 12px transparent shadow margin so drag/resize hit zones align), the overlay grows from 920×360 to 920×640 with a fixed top edge when a run starts and returns to compact on New Chat/re-summon, post-capture re-shows preserve the exact frame instead of snapping to the mouse, and the model picker closes on Escape and outside clicks | done | 2026-07-02 |
| main | Workflow simplification — rewrote `docs/WORKFLOW.md` as a trunk-based flow (main only, checks matched to changed files, branch only for risky multi-day work, releases as `v*` tags), removed dead Railway deployment (railway.toml, nixpacks.toml, RAILWAY_CLI.md), and deleted the stale `test` branch plus 15 merged feature branches | done | 2026-07-02 |
...
| main | Grok vision reality check — verified the Grok subscription proxy directly, kept `grok-build` vision-enabled, reverted Composer to text-only because upstream rejects image input, and added Grok png/jpeg plus minimum-pixel validation before requests hit the proxy | done | 2026-06-26 |
| main | Grok OAuth vision capability pass — initially enabled subscription-backed Grok vision metadata after confirming the proxy uses Responses image input; superseded by the Grok reality-check row above for Composer-specific behavior | done | 2026-06-26 |
| main | ChatGPT OAuth vision capability fix — aligned subscription-backed ChatGPT providers with registry vision/tool metadata so image attachments pass backend validation, while preserving Responses `input_image` translation coverage | done | 2026-06-26 |
| main | Image input visibility — raised chat and `view_image` limits to 20 images, persisted sent image attachments on run records, surfaced clickable thumbnails/previews for user messages and live image-tool results, and carried `supports_vision` through context profile typing | done | 2026-06-26 |
| main | modify coding context core — added normalized coding-session replay with tool-call/tool-output invariant repair, orphan output dropping, unsupported-media stripping, durable context-window state in coding sessions, context pressure/window tracing, explicit `get_context_remaining` and `new_context_window` tools, safer forced compaction window advancement, richer regenerated coding context metadata, and tests for replay normalization/window state/context tools | done | 2026-06-26 |
| main | Local agent sampling defaults — lowered config-driven generation temperature to 0.2, set top_p to 0.9, and wired agent runs to pass top_p plus config temperature through to selected/local models so deterministic coding-agent decoding is actually used outside the UI | done | 2026-06-21 |
| main | Tool runtime hardening — command approval parses compound segments while preserving relaxed read auto-approval; command sessions now emit incremental stable chunks, support explicit termination, block finality while unresolved, and persist success/failure logs; live write/edit replacement is atomic; grep errors are truthful; discovery prunes ignored directories and supports nested ignores; argument validation is recursive; images use signature checks; mixed reads are labeled pre-mutation; and the live legacy Bash tool was removed behind a compatibility input alias | done | 2026-06-21 |
| main | Codex-style shell hardening — removed python_execute/Daytona from live config/API/tool registration, made exec_command/write_stdin the primary shell surface, added bounded head/tail command output with Codex-style metadata, process-group termination, PTY-only arbitrary stdin, artifact payload replay, streamed artifact reads, malformed-tool retry enforcement, command-session UI updates, sanity command verification alignment, and stronger web/image failure validation | done | 2026-06-20 |
| main | Agent tool hardening — centralized workspace path containment and gitignore handling across file tools, made glob/grep counts deterministic, fixed glob recursion/case-insensitive matching semantics, removed hidden read_file auto-continue state, preserved prompt-visible payloads on duplicate read/search calls, added tool schema validation before execution, improved long-line truncation, shallow list_directory behavior, prompt/UI-visible glob/grep/list payload preservation, and web_extract partial-failure reporting | done | 2026-06-20 |
| main | Desktop terminal/browser panel stability — fixed stale terminal recovery to be restart-only, hydrated xterm replay per session instead of falling back to conversation-level buffers so stale/new terminals cannot inherit old output, reserved a native-webview-safe tools-panel resize gutter, hid Browser webviews during divider drags, and throttled panel/browser bounds updates through requestAnimationFrame for smoother resizing | done | 2026-06-20 |
| main | Landing theme dropdown click fix — raised the fixed navigation stacking context above main content so the theme menu remains interactive after extending below the header | done | 2026-06-20 |
| main | Three-way theme preferences — replaced the landing-site and macOS app theme toggles with accessible Light/Dark/System dropdowns using icons, selected-state checks, outside-click/Escape dismissal, and arrow-key navigation; System is now an explicit persisted preference that follows live OS appearance, including native Tauri window chrome, while existing saved Light/Dark choices remain compatible | done | 2026-06-20 |
| main | Desktop light mode — added a no-flash OS-aware theme bootstrap plus persisted sun/moon override in the sidebar header, carried the landing site's warm-greige palette through the complete Tauri shell (thread, composer, dialogs, settings, HUD, approvals, tool diffs, toasts, browser-tool chrome, syntax highlighting, and xterm), made Tailwind neutral/status utilities and desktop CSS surfaces theme-aware while preserving the existing dark appearance, and synchronized native window chrome through Tauri's theme API | done | 2026-06-19 |
| main | Agent HUD presence pass — gave the desktop live/running state real presence as a flat card with a cyan "spine" that echoes the agent steps progress spine (muted grey when inactive/terminal), the soft ring-pulse status dot (`desktop-step-dot-pulse`), a quiet fade on the rotating phase word, and structured flat stat chips (ctx/time/tok, ctx turns amber past the warning threshold); unified the plan/permission/input approval panels into the same card+spine language with the spine colored by kind (violet plan/input, amber permission) plus a quiet entrance; web live dot pulses while active; stays flat/mono-cyan with reserved state colors (no gradients/scan-lines/glows/phase-rainbow), all motion gated under `prefers-reduced-motion`; also fixed the plan-approval view rendering raw markdown in a `<pre>` by routing it through the shared `AnswerMarkdown` renderer inside a scrollable inset surface (diff-style plan revisions still use `UnifiedDiffView`) | done | 2026-06-14 |
...
| main | Desktop dev startup fix — added a project-local pnpm workspace config anchored to the UI package, disabled pnpm v11 dependency auto-install checks during script runs, approved the known `sharp` build script, defaulted uv to the repo-local cache, and started the desktop API without uvicorn reload so `./dev.sh desktop` avoids pnpm/uv permission failures | done | 2026-06-11 |
| main | Durable completion gate — removed web-search nudging and prose-pattern progress detection, added model-agnostic finality gating from structured tool calls plus run evidence so intermittent no-tool updates stay non-final until required workspace/web/calculation/command evidence is satisfied, with traceable gate decisions and protocol-stall protection | done | 2026-06-11 |
| main | Landing mobile polish — tightened the fixed nav, hero typography, CTA sizing, demo video spacing/shadow, single-column provider/features grids, section rhythm, install command wrapping, and footer stacking for phone widths inspired by the SpotifyTray landing page | done | 2026-06-10 |
| main | Composer progress/tool recovery — assistant text bundled with structured tool calls now renders as non-final progress, explicit research tasks get one API-level forced `web_search` recovery when a model emits progress without a tool call, forced-tool failures stop with a clear protocol error instead of nudging loops, and Responses streaming parsing now covers official function-call/reasoning event variants | done | 2026-06-07 |
| main | OpenAI/ChatGPT Responses compatibility — custom function tools now explicitly opt out of strict Responses schema normalization, orphan tool outputs are replayed as text context so OpenAI does not reject missing call IDs, ChatGPT subscription models now use current GPT-5.5/5.4 IDs with stale selections falling back safely, provider HTTP errors surface real API details, and live OpenAI/xAI catalogs hide stale curated IDs | done | 2026-06-06 |
| main | Landing hero demo video — replaced the static Fluxion hero screenshot with the new compressed `flux_demo-3` MP4 product demo as a flat muted autoplay hero video with no player controls or perspective tilt, updated the browser tab title plus hero heading to “Coding agent for the models you choose,” and clarified the subtitle around local models, APIs, or subscriptions | done | 2026-06-04 |
| main | Desktop browser overlay/workspace polish — native Browser webviews now hide while app dialogs are open so model/settings pickers are not buried, active browser bounds resync continuously to follow sidebar/panel layout changes, draft-to-real run startup preserves the open tools panel and keeps Browser WebViews stable while attaching the terminal/browser state to the real conversation, and New Workspace clears the selected conversation before setting the draft folder so the chosen workspace sticks on the first selection | done | 2026-06-04 |
| main | Parallel key UI + empty-window drag/workspace/terminal draft fix — model/settings picker now exposes standalone tool-provider API keys like Parallel for web search/extract, the desktop empty first-launch screen is a native drag surface while suggestion buttons stay clickable, New Workspace no longer races the old route, and draft workspace chats can use terminal/browser sessions before the first message then attach them to the created conversation on send | done | 2026-06-04 |
| main | Explicit provider selection no-fallback — registry resolution now rejects explicit provider selections without credentials instead of silently falling back to another configured provider, so selecting unavailable OpenRouter/xAI/etc. cannot run through Fireworks under the wrong label | done | 2026-06-03 |
| main | Explicit model provider routing — chat and agent runs now preserve the selected provider in `X-Provider`/`X-Model` routing, so Grok OAuth `grok-build` resolves as `grok:grok-build` instead of falling back to the default Fireworks provider, and selected-model resolution failures now fail visibly instead of silently using a different provider | done | 2026-06-03 |
| main | macOS release version sync — release builds now stamp bundled `Info.plist` with tag marketing version plus the same numeric build used by Sparkle, pass the version into the Tauri shell/backend, and fall back to the latest v-tag for local post-tag builds so DMG/Homebrew installs report the same app version | done | 2026-06-03 |
| main | Desktop packaged interaction fixes — added packaged-app Grok CLI discovery/freeze hardening, kept Grok Connect from staying disabled during browser OAuth, added perimeter drag strips for frameless desktop windows, and made repeated New Workspace selections reset the blank draft even when the same folder is chosen | done | 2026-06-03 |
