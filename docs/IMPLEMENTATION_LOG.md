...
|--------|-------------|--------|---------|
| main | New Workspace draft selection fix — New Workspace now flush-navigates off `/conversations/:id` before clearing selection/writing the draft workspace so route sync cannot reselect the old URL conversation, and ConversationView only mirrors an existing conversation workspace when that conversation is still selected at effect time | done | 2026-06-13 |
