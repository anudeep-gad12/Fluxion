...
|--------|-------------|--------|---------|
| feature/coding-session-state | Coding-session continuity overhaul — added persistent per-conversation coding session state for coding-profile runs, restored durable file evidence/validation/open-task state across turns, reused fresh inspected-file excerpts instead of summary-only reruns, added stale-file detection plus targeted reread guidance, persisted session snapshots through tool progress/run completion, and added storage/agent/integration regressions for follow-up coding turns | done | 2026-04-30 |
